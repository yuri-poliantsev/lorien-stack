...
The capture lever waits on `theme-canvas` and `theme-unit` with poses `working | idle | sleeping`. A working still shoots when at least `ceil(N / 3)` units are `working`. A 40 s timeout exits 1 and names theme, N, need, and the pose counts. The manifest line records `pose_working= pose_idle= pose_sleeping=` from those unit poses.
...
pose_working / pose_idle / pose_sleeping  (theme-unit data-pose)
```

The header strip and those pose columns come from different clocks and will not match.
