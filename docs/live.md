...
| Bots appear, activity stays empty | A transcript path is invalid, or its lines do not use the Grok transcript shape. | Use `agent-transcripts/<bot-uuid>/<name>.jsonl`. Each line must look like `{"role":"...","message":{"content":[...]}}`. The first catch-up uses the file mtime. Later lines use observation time. |
