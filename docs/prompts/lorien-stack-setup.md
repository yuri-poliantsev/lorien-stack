...
Paste the block below into a chat with a Grok Bot that runs on the machine holding `$AGENT_DATA`. The bot clones the repo, writes `AGENT_DATA`, starts both processes, and hands back a tailnet URL.
...
If a step fails, the symptom is probably in the troubleshooting table in [Live setup](../live.md). An empty roster means the data path is wrong. A blank UI with a healthy `/api/bots` means the client is not reaching `/ws`.
